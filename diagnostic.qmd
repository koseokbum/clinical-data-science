# 진단 도구의 성능 측정([performance measure]{.style-text font-weight="bold" color="#023FA5"}) {#section-diagnostic}
...
::: {.callout-caution}
Confusion matrix에서 각 칸에 들어가는 값은 카운트(count)라는 사실을 잊지 않아야 한다. 즉, 각 칸에 들어가는 값은 해당 조건을 만족하는 환자의 수를 의미한다. 예를 들어, `TP`는 질병이 있는 환자 중에서 검사 결과가 양성인 환자의 수를 의미한다. 만약 Sensitivity를 99%라고 했을 때 TP에 99, FN에 1이 들어가서는 안 된다. 
:::


이 행렬을 사용하여 진단 도구의 민감도와 특이도를 계산할 수 있다.
...
![특이도: 질병이 없는 사람에서 검사가 음성일 확률](images/specificity.png){#fig-specificity fig-align="center" width="80%"}
...
혼동 행렬을 사용하여 수많은 지표들은 많다. 공식을 외우는 것보다는 이해하는 데 초점을 맞출 필요가 있다.
